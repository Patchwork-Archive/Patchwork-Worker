...
def rclone_to_cloud():
    subprocess.run(f'{CONFIG.get("path","rclone_path")} -P copy "{CONFIG.get("path","download_output_path")}" "{CONFIG.get("path","rclone_cloud_target")}"')
...
def download_and_upload():
    ...
    file_util.clear_output_folder(CONFIG.get("path","download_output_path")) if input() == "y" else print("Skipping...")
    ...
    data_converter.generate_json_data(video_input_dir = "output_video",file_type = ".webm", ndjson_path= CONFIG.get("path", "ndjson_path")) if input() == "y" else print("Skipping...")
...
def _make_commit_message():
    ...
    for video in data_converter.get_all_files_in_directory(CONFIG.get("path","download_output_path"), file_type = "webm"):
    ...
    while cmd != Options.EXIT.value:
        match cmd:
            ...
            case Options.COMMIT_TO_REMOTE.value:
                data_converter.commit_and_push(CONFIG.get("path", "git_repo_path"), _make_commit_message())
