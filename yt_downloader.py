    ...
    def download_urls(self):
        ...
        def _extract_video_id_from_url(url: str):
            ...
            if "youtube.com/watch?v=" in url:
                ...
                return url.split("https://youtube.com/shorts/")[1].replace("?feature=share", "")
            

        def _download_youtube_url(url_id: str):
            output_path = os.path.join(self._output_dir, f"{url_id}")
            print(
                "Executing", f'yt-dlp {url}{url_id} -f bestvideo[height<=1080][ext=webm]+bestaudio -o "{self._output_dir}/%(id)s.%(ext)s" --add-metadata')
            subprocess.run(
