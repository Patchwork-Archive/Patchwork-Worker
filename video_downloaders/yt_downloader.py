...
class YouTubeDownloader(VideoDownloader):
    ...
    def download_video(self, video_url: str, file_type:str="webm"):
        ...
        if os.path.getsize(f"{self._output_dir}/video/{self._get_video_id(video_url)}.webm") > self._max_file_size_bytes:
            self._write_debug_log(f"Video {video_url} exceeds max file size. Deleting...")
            self._write_to_log_deleted(video_url)
            os.remove(f"{self._output_dir}/video/{self._get_video_id(video_url)}.webm")
            return
