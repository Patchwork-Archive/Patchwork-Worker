...
class YouTubeDownloader(VideoDownloader):
    ...
    def download_video(self, video_url: str, file_type: str = "webm"):
        ...
        result = subprocess.run(
            f'yt-dlp "{video_url}" --cookies cookies.txt -f "bestvideo[height<=1080][ext=webm]+bestaudio" -o "{self._output_dir}/video/%(id)s.%(ext)s" --add-metadata',
            shell=True,
        )
        ...
        if result.returncode != 0 or not os.path.exists(video_path):
            ...
            result = subprocess.run(
                f'yt-dlp "{video_url}" --cookies cookies.txt -f "bestvideo[height<=1080][ext=mp4]+bestaudio" -o "{self._output_dir}/video/%(id)s.%(ext)s" --add-metadata',
                shell=True,
            )
    ...
    def download_thumbnail(self, video_url: str):
        ...
        subprocess.run(
            f'yt-dlp "{video_url}" --cookies cookies.txt --write-thumbnail --skip-download --convert-thumbnails jpg -o "{self._output_dir}/thumbnail/%(id)s.%(ext)s"',
            shell=True,
        )
    ...
    def download_captions(self, video_url: str):
        def get_non_autogenerated_subs(url):
            ...
            try:
                output = subprocess.check_output(['yt-dlp', '--cookies', 'cookies.txt', '--list-subs', url], stderr=subprocess.STDOUT)
        ...
        def download_subs(url, subs, video_id):
            ...
            for sub in subs:
                ...
                try:
                    subprocess.check_output(['yt-dlp', '--cookies', 'cookies.txt', '--sub-format', 'srv3', '--sub-lang', sub, '--write-sub',
    ...
    def download_metadata(self, video_url: str) -> dict:
        ...
        subprocess.run(
            f'yt-dlp --cookies cookies.txt --write-info-json -o "output/metadata/%(id)s.%(ext)s" --skip-download {video_url}',
            shell=True,
        )
